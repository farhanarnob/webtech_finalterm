<?php include "../include/header.php" ?>
<form  id="submission_answer_form" method="POST">
	<div id="main">
		<form>
<h2>1. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
<form>
<h2>2. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
<h2>4. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
</form>
<h2>5. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
<form>
<h2>6. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
<h2>7. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
</form>
<h2>8. Which of the following methods cannot be used to enter data in a cell </h2>
  <input type="radio" name="gender" value="male" checked> Pressing an arrow key<br>
  <input type="radio" name="gender" value="female"> Pressing the Tab key<br>
  <input type="radio" name="gender" value="other"> Pressing the Esc key<br>
  <input type="radio" name="gender" value="other"> Clicking on the formula bar<br>
</form>
	</div>
	<div id="submission_div">
		<button onclick="submission_answer()" id="submit_button">SUBMIT NOW</button>
	</div>
	<div id="timer_question">
		<p id="current_time">Start</p>
		<script>
			timee();
		</script>
	</div>



</form>
<?php include "../include/footer.php" ?>